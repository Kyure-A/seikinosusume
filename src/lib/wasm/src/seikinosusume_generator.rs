...
fn round_icon(icon: DynamicImage) -> DynamicImage {
    ...
    let resized_icon: ImageBuffer<Rgba<u8>, Vec<u8>> = resize(&rounded_icon, 755, 755, image::imageops::FilterType::CatmullRom);
...
fn draw_icon(mut image: DynamicImage, mut icon: DynamicImage) -> DynamicImage {
    ...
    image::imageops::overlay(&mut image, &icon, 167, 195);
    return image;
}

fn seikinosusume_generator(icon: ImageBuffer<Rgba<u8>, Vec<u8>>, seikinga: String, seikin: String) -> ImageBuffer<Rgba<u8>, Vec<u8>> {
    let mut image: DynamicImage = image::open("static/base.png").unwrap();
    let dynamic_icon: DynamicImage = DynamicImage::ImageRgba8(icon);
    
    image = make_seikin(&seikin, image.clone());
    image = make_seikinga(&seikinga, image.clone());
    image = draw_icon(image.clone(), dynamic_icon.clone());
...
pub fn exec(buffer: Vec<u8>, width: u32, height: u32, seikinga: String, seikin: String) -> Vec<u8> {
    let icon: ImageBuffer<Rgba<u8>, Vec<u8>> = ImageBuffer::<Rgba<u8>, Vec<u8>>::from_raw(width, height, buffer).unwrap();
    return seikinosusume_generator(icon, seikinga, seikin).into_raw();
}
